...
//class CardGame {
//    String[] cards; //카드 정보
//    int players_cnt = 0;// 플레이어 수
//    String[][] player_card;//플레이어가 가지고 있는 카드 정보
//    int[] player_score; //플레이어 총 점수 배열
//
//    CardGame(int players_cnt) {
//        this.players_cnt = players_cnt; //플레이어 숫자
//        this.player_score = new int[players_cnt];//플레이어 점수 합계 저장
...
//        //1) 카드를 1차원 배열에 초기화 한다.
//        for (int j = 0; j < cards.length; j++) { //1~52장까지 카드 초기화
...
//
//    void play() { //카드 게임 플레이
//        int card_index = 0; //cards[]의 배열 index
//        int sum = 0; //카드 숫자 합계
//        //카드 배분
//        for (int i = 0; i < players_cnt; i++) {//3) 각 플레이어에게 7장씩 나누어준다.
...
//            }
//        }
//        int winner = WhoisWinner(player_score);//우승자 번호를 저장한다.
//        Boolean check = CheckManyWinner(winner - 1); //winner-1 = winner_index
...
//    }
//
//    int WhoisWinner(int[] player_score) {
//        //우승자 2명 이상있는지 판별하는 메서드
//        int min = player_score[0];//최소 점수
//        int winner_index = 0;
//        for (int i = 1; i < player_score.length; i++) {
//            if (min > player_score[i]) {//플레이어 점수가 더 작으면 tmp 값과 winner_index(우승자)값을 변경한다.
//                min = player_score[i];
//                winner_index = i;
//            }
//        }
//        return winner_index + 1;// 플레이어 번호 = index+1
//    }
...
//        int player_cnt = 4; //플레이어 인원 수(1~6명의 게임 인원수 가정)
